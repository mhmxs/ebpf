...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc clang-11 TopWriteExample ./bpf/topwrite_example.c -- -nostdinc -I../headers
...
func main() {
	...
	objs := TopWriteExampleObjects{}
	if err := LoadTopWriteExampleObjects(&objs, nil); err != nil {
	...
	kp, err := link.Kprobe(fn, objs.KprobeGenericWriteEnd)
	if err != nil {
	...
	for {
		...
		log.Printf("pid: %d\tcomm: %s\t iname: %s\n", event.PID, unix.ByteSliceToString(event.Comm[:]), unix.ByteSliceToString(event.Iname[:]))
	}
}
